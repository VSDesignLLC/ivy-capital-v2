...
(function(){
  ...
  function def(name, html){
    ...
    customElements.define(name, class extends HTMLElement{
      connectedCallback(){
        ...
        /* 语言切换 = 当前页的中/英对应版（X-ch.html ↔ X-en.html）；无 -ch/-en 后缀的页保留默认 home 链接 */
        var file = (location.pathname.split('/').pop()||'');
        var m = file.match(/^(.*?)-(ch|en)\.html$/);
        if(m){
          var zl=this.querySelector('.nav__lang a.z'), enl=this.querySelector('.nav__lang a.en');
          if(zl) zl.setAttribute('href', m[1]+'-ch.html');
          if(enl) enl.setAttribute('href', m[1]+'-en.html');
        }
        /* 线上惰化未部署的英文页链接（EN 页仍在 .vercelignore 中，勿让线上 EN 切换落到 404） */
        if(!isLocal){ var ens=this.querySelectorAll('.nav__lang a.en'); for(var i=0;i<ens.length;i++){ ens[i].removeAttribute('href'); } }
      }
    });
  }
  ...
  var NAV_V2_EN = navV2([["about-en.html","about","About"],["team-en.html","team","Team"],["portfolio-en.html","portfolio","Portfolio"],["news-en.html","news","News"]], "homepage-en.html", "Ivy Capital");
